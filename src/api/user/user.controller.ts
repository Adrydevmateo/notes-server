...
export class UserController {
	...
	async CreateUser(@Body() reqBody: TUserDTO): Promise<TUserCRUDResponse> {
		...
		if (!created.OK) return { msg: created.msg, statusCode: HttpStatus.UNAUTHORIZED, OK: false }
	...
	async SignIn(@Body() reqBody: TUserDTO): Promise<TUserCRUDResponse> {
		...
		if (!authorized.OK) return { msg: authorized.msg, statusCode: HttpStatus.UNAUTHORIZED, OK: false }
	...
	async UpdateUser(@Body() reqBody: TUserDTO): Promise<TUserCRUDResponse> {
		...
		if (!updated.OK) return { msg: updated.msg, statusCode: HttpStatus.UNAUTHORIZED, OK: false }
	...
	async DeleteUser(@Body() reqBody: { id: string, token: string }): Promise<TUserCRUDResponse> {
		...
		if (!deleted.OK) return { msg: deleted.msg, statusCode: HttpStatus.UNAUTHORIZED, OK: false }
